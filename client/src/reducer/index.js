...
export default function rootReducer(state=initialData, action){
    switch(action.type){
        ...
        case GET_POKEMONES:
            return{...state, pokemonsLoaded: action.payload.api, myPokemons:action.payload.db}
        ...
        case FILTER_ORIGIN_POKEMON:
                if(action.payload === 'created'){
                    ...
                    return{...state, pokemonsFiltered: state.pokemonsLoaded.concat(state.myPokemons)}
                }
        case FILTER_TYPE_POKEMON:
            if(action.payload === 'all'){
                return{...state,pokemonsShowed: state.pokemonsFiltered}
            }else{
                return{...state,pokemonsShowed: state.pokemonsFiltered.filter(el => el.types.includes(action.payload))}
            }
        case SORT_POKEMONS:
                if(action.payload === 'low-high'){
                    console.log('Sorting')
                    return {...state, pokemonsShowed: state.pokemonsShowed.sort((a,b)=>{
